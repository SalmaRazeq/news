class AssetsManager {
  static const String base = 'assets/images/';
  static const String splash = '${base}splash.png';
  static const String pattern = '${base}pattern.png';
  static const String sport = '${base}sports.png';
  static const String general = '${base}Politics.png';
  static const String health = '${base}health.png';
  static const String business = '${base}business.png';
  static const String technology = '${base}technology2222.png';
  static const String science = '${base}science.png';
  static const String entertainment = '${base}entertainment.png';

}